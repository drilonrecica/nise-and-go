...
| `crypto/hmac`, `crypto/sha256`, `net/http` (standard library) | AWS Signature Version 4 and the S3 client, in `internal/platform/storage`. The alternative is the AWS SDK: a very large amount of generated code, a credential chain, retry middleware, and a plugin interface, for four HTTP verbs. The algorithm is specified precisely and does not change, and it is verified against a real S3-compatible service rather than only against itself. |
| `net/smtp` (standard library) | The SMTP client behind `internal/platform/mail`. It is frozen rather than actively developed, and that is acceptable here: SMTP is likewise frozen, the package covers STARTTLS, implicit TLS, and AUTH, and the alternative is a dependency for a protocol whose whole client fits in one file. What it does not do — dot stuffing aside — is handled in this repository's own code, with tests asserting the bytes on the wire. |
...
|---|---|
| `@sveltejs/kit` (`2.70.3`), `svelte` (`5.56.10`) | The generated frontend framework, on Svelte 5 runes only. The authenticated application is embedded in the Go binary, so it ships as one artifact; protected routes are never prerendered and universal `load` functions run in the browser. |
| `@sveltejs/adapter-static` (`3.0.10`) | The adapter that makes the above a directory of files rather than a server. There is one server in this architecture and it is written in Go; an adapter that needed Node in production would add a second process to operate for no capability the application uses. |
| `@sveltejs/vite-plugin-svelte` (`7.3.0`), `vite` (`8.2.2`) | The compiler plugin and the bundler SvelteKit is built on. Neither is a separate choice — they are what the framework above requires — and both are pinned rather than floated so two builds of one commit produce the same bundle. |
| `tailwindcss` (`4.3.3`), `@tailwindcss/vite` (`4.3.3`) | The generated styling system and its bundler plugin. Its output is an external stylesheet, which is what lets the Content Security Policy keep `style-src` free of `'unsafe-inline'` ([ADR 0013](adr/0013-security-headers-and-csp.md)). Released in lockstep, so both carry one pin. |
| `svelte-check` (`4.7.6`) | Type-checks Svelte components, which `tsc` alone does not: the types inside a `.svelte` file are not TypeScript the compiler can see. Part of `pnpm check`, and a build tool — nothing from it ships. |
| `@biomejs/biome` (`2.5.11`) | Linting and formatting for the frontend, in one tool rather than the customary two plus a plugin to make them agree. A test and build dependency; see [linting](linting.md). |
| `@playwright/test` (`1.62.1`) | The end-to-end test runner, pinned in lockstep with the `playwright` driver below so a project cannot end up running one version against another. |
