...
- Both the Go and the pnpm tree sit inside the module, so `./...` and `gofmt -l .` walk `frontend/node_modules/` — measured, not assumed. An npm package that ships a `.go` file is listed as a package of the application. Generated tooling therefore names its Go targets explicitly rather than relying on `./...`: `./cmd/... ./internal/...` today, joined by `./db/...` once `db/embed.go` exists.
