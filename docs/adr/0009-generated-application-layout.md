...
| Problem Details policy | `internal/platform/httpapi/problem/` | written once | application |
...
Files Nise renders directly carry one of two exact headers, in the comment syntax of their language:
...
The first form matches Go's generated-code convention so linters and diff tools skip it. Neither Nise header records a version: a version there would rewrite every file on every CLI bump and break the clean-regeneration check. Checked-in output from an external pinned generator retains that generator's standard `Code generated … DO NOT EDIT.` header; changing it would make a normal regeneration dirty, and `store/`, `openapigen/`, or the `.gen.go` suffix already declares structural ownership. JSON files, which have no comment syntax, declare ownership by path instead. So do the message templates under `internal/platform/mail/templates/`, for a stronger version of the same reason: their plain-text part is sent verbatim to a person, so a header in one would arrive in somebody's inbox. That directory's `README.md` is the declaration for every file in it.
...
- Both the Go and the pnpm tree sit inside the module, so `./...` and `gofmt -l .` walk `frontend/node_modules/` — measured, not assumed. An npm package that ships a `.go` file is listed as a package of the application. Generated tooling therefore names its Go targets explicitly rather than relying on `./...`: `./cmd/... ./db/... ./internal/...` now that the migration embed package exists.
