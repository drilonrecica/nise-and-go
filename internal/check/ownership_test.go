...
// TestOpenAPIGeneratedFileRequiresThePinnedToolHeader keeps the external
// generator exception narrow: only its exact path and pinned version count
// as the expected ownership marker.
func TestOpenAPIGeneratedFileRequiresThePinnedToolHeader(t *testing.T) {
	t.Parallel()

	root := newProject(t)
	body := readFile(t, root, generator.OpenAPIGeneratedPath)
	writeFile(t, root, generator.OpenAPIGeneratedPath,
		strings.Replace(body, generator.OAPICodegenGeneratedHeader,
			"Code generated by github.com/oapi-codegen/oapi-codegen/v2 version v0.0.0 DO NOT EDIT.", 1))

	c := assertStatus(t, run(t, root), check.CheckOwnershipHeaders, check.StatusFail)
	if !containsPath(c.Paths, generator.OpenAPIGeneratedPath) {
		t.Errorf("paths = %v, want it to name the file generated by the wrong tool version", c.Paths)
	}
}
...
func TestAppOwnedFileGainingTheNiseHeaderFails(t *testing.T) {
	for _, marker := range []string{
		...
		t.Run(marker, func(t *testing.T) {
			...
			root := newProject(t)
			writeFile(t, root, "internal/app/app.go", "// "+marker+"\n\npackage app\n")
